    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long currentTime) {
            ...
            if (j < 0 || currentTime < j) {
                return AutoScrollHelper.constrain(((float) (currentTime - elapsedSinceEnd)) / ((float) this.mRampUpDuration), 0.0f, (float) AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY) * 0.5f;
            }
            long elapsedSinceEnd2 = currentTime - j;
            float f = this.mStopValue;
            return (AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY - f) + (f * AutoScrollHelper.constrain(((float) elapsedSinceEnd2) / ((float) this.mEffectiveRampDown), 0.0f, (float) AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY));
        }
